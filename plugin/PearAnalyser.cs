...
namespace AssemblyCSharp {
	...
	public class PearAnalyser : MonoBehaviour {
		...
		//calculate a framerate when required and record it in the active session
		private void CalculateFrameRate() {
			int frameRate;
			frameCounter++;
			timeCounter += Time.deltaTime;
			globalTimeCounter += timeCounter;
			//test to respect the limit of updates per second
			if(timeCounter > 1.0f / UpdatesPerSecond) {
				...
				//create a new metric and add it to the active session
				session.createMetric(new Metric("fps", frameRate, (uint) (globalTimeCounter * 1000)));
			}
		}

		//send a POST request to the server url, with the JSON-formated active session
